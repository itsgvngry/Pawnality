...
public class MainFrame extends JFrame {
    ...
    // Question screen refs (updated each question)
    private JLabel    qCounterLabel;
    private JLabel    qTextLabel;
    private JButton[] answerBtns = new JButton[4];
    private JProgressBar progressBar;

    // Result screen refs (updated when quiz ends)
    private JLabel    resPieceLabel;
    private JLabel    resCongLabel;
    private JLabel    resNameLabel;
    private JLabel    resTraitLabel;
    private JLabel    resDescLabel;
    private JLabel    resGmLabel;
    private JLabel    resGmTitleLabel;
    private JLabel    resGmStyleLabel;
    ...
    private JPanel buildWelcomeScreen() {
        ...
        ActionListener onStart = e -> {
            try {
                QuizEngine.validateName(nameField.getText()); // EXCEPTION HANDLING
        ...
        nameField.addActionListener(onStart); // also triggers on Enter key
    ...
    private JPanel buildResultScreen() {
        ...
        // History button — shows saved results using FILE HANDLING
    ...
    private void handleAnswer(int index) {
        ...
        engine.submitAnswer(q.getPlaystyleKey(index)); // record score
    ...
    private void showResult() {
        String key       = engine.calculateResult();        // uses RECURSION
        Playstyle result = QuizEngine.createPlaystyle(key); // POLYMORPHISM
        ...
        ResultFileManager.saveResult(userName, result.getName()); // FILE HANDLING

        cardLayout.show(mainPanel, "result");
    }

    private void showHistory() {
        List<String> history = ResultFileManager.loadHistory(); // FILE HANDLING read
        if (history.isEmpty()) {
    ...
    // Gold filled button used for primary actions
    ...
    // Gold outlined button used for secondary actions
    ...
    // Answer button with gold letter badge on the left
    private JButton answerBtn(final String letter, int index) {
        ...
        btn.setAlignmentX(Component.CENTER_ALIGNMENT);
        btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        btn.addActionListener(e -> handleAnswer(index)); // EVENT HANDLING
        return btn;
    }

    // ═══════════════════════════════════════════════════════════════════════
    //  QUESTION DATA  (10 scenario-based questions, ARRAY of Question objects)
    // ═══════════════════════════════════════════════════════════════════════
